...
/* ___ Read Global Definitions _______________________________________________*/
    ...
    if ( otf2_DOT )
    {
        /* OTF2_Reader_Open ensures a '.' extension and only supports '.otf2' */
        size_t dot_path_size = strlen( anchor_file ) + strlen( ".SystemTree.dot" ) + 1;
        dot_path = malloc( dot_path_size );
        check_pointer( dot_path, "Allocating path for DOT file path." );
        snprintf( dot_path, dot_path_size, "%.*s.SystemTree.dot",
                  ( int )strlen( anchor_file ) - 5, anchor_file );
