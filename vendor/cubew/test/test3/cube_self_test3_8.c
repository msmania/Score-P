...
void
create_cube_structure( cube_t*       cube,
                       cube_metric** metrics,
                       cube_cnode**  cnodes,
                       unsigned      Nthreads );
void
...
main( int argc, char* argv[] )
{
    ...
    unsigned Nlocations = NLGS * NTHREADS; // / In this pattern, number of threads
    ...
    char* cubefile = "gpu-metrics-cube";
    ...
    cube_type_double     sev_double[ Nlocations ];
    cube_type_int64      sev_int64[ Nlocations ];
    cube_type_tau_atomic sev_atomic[ Nlocations ];
    ...
    for ( mi = 0; mi < NMETRICS; mi++ ) // / run over the metrics
    {
        ...
        {
            if ( CUBE_DATA_TYPE_DOUBLE == metrics_vector[ mi ]->dtype_params->type )
            {
                fill_cube_type_double_row( sev_double, Nlocations, ( double )( ci + mi ) );
                cube_write_sev_row_of_cube_type_double( cube, metrics_vector[ mi ], ( cube_cnode* )( sequence->data )[ ci ], sev_double );
            }
            if ( CUBE_DATA_TYPE_INT64 == metrics_vector[ mi ]->dtype_params->type )
            {
                fill_cube_type_int64_row( sev_int64, Nlocations, ( double )( ci + mi ) );
    ...
    for ( mi = 0; mi < NTAUMETRICS; mi++ )  // / run over the metrics
    {
        ...
        for ( ci = 0; ci < sequence->size; ci++ )
        {
            fill_cube_type_tau_atmic_row( sev_atomic, Nlocations, ( double )( ci + mi ) );
...
fill_cube_type_double_row( cube_type_double* row, unsigned size, double value )
{
    ...
    for ( i = 0; i < size; i++ )
    {
        row[ i ].value = value + ( double )i;
    }
}


void
fill_cube_type_int64_row( cube_type_int64* row, unsigned size, unsigned value )
{
    unsigned i = 0;
    for ( i = 0; i < size; i++ )
    {
...
void
fill_cube_type_tau_atmic_row( cube_type_tau_atomic* row, unsigned size, double value )
{
    unsigned i = 0;
    for ( i = 0; i < size; i++ )
    {
        double A = value + ( double )i;
...
//      cube_region *regn0, *regn1, *regn2;
    ...
    cube_system_tree_node* stn0      = cube_def_system_tree_node( cube, "MSC", "", "machine", NULL );
    cube_system_tree_node* stn1      = cube_def_system_tree_node( cube, "Athena", "", "node", stn0 );
    cube_system_tree_node* stn2      = cube_def_system_tree_node( cube, "Tesla V100", "", "gpu device", stn1 );
    cube_location_group*   lg0       = cube_def_location_group( cube, "Process", 0, CUBE_LOCATION_GROUP_TYPE_PROCESS, stn1 );
    cube_location_group*   lg_metric = cube_def_location_group( cube, "Metrics", 0, CUBE_LOCATION_GROUP_TYPE_METRICS, stn1 );
    cube_location_group*   lg_gpu    = cube_def_location_group( cube, "CUDA device", 0, CUBE_LOCATION_GROUP_TYPE_ACCELERATOR, stn2 );
    char                   name_thread[ 23 ];
    char                   name_metric[ 26 ];
    char                   name_gpu[ 23 ];
    ...
    {
        snprintf( name_thread, 23, "Thread %u", i );
        snprintf( name_metric, 26, "Metric %u", i );
        snprintf( name_gpu, 23, "CUDA stream %u", i );

        cube_def_location( cube, name_thread, 3 * i, CUBE_LOCATION_TYPE_CPU_THREAD, lg0 );
        cube_def_location( cube, name_metric, 3 * i + 1, CUBE_LOCATION_TYPE_METRIC, lg_metric );
        cube_def_location( cube, name_gpu, 3 * i + 2, CUBE_LOCATION_TYPE_ACCELERATOR_STREAM, lg_gpu );
    }
