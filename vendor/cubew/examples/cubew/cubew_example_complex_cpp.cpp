...
main( int argc, char* argv[] )
{
    ...
    std::string cubefile( "complex-cube-cpp" );
    char*       cubefile_name = ( char* )calloc( cubefile.size() + 1, sizeof( char  ) );
    cubefile.copy( cubefile_name, cubefile.size() );

    //    FILE* fp = fopen(cubefile, "w");
    cube_t* cube = cube_create( cubefile_name, CUBE_MASTER, CUBE_FALSE );
    if ( !cube )
    {
...
//      cube_region *regn0, *regn1, *regn2;
    ...
    for ( i = 0; i < 169; i++ )
    {
        ...
        regn[ i ] = cube_def_region( cube, names[ i ].c_str(), names[ i ].c_str(), "mpi", "barrier",  -1, -1, "", descr.c_str(), mod.c_str() );
    }
