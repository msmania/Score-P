      ...
      char** argv )
{
    ...
    bool        nvcc   = false;
    std::string summary_command( "cat " PKGDATADIR "/cubew.summary" );
    ...
    for ( int i = 1; i < argc; i++ )
    {
        ...
        {
            out( string( "version " ).append( CUBEW_VERSION ) );
            exit( EXIT_SUCCESS );
        }
                  ...
                  || !strncmp( argv[ i ], "--features=", 11 ) )
        {
            ...
            {
                die( string( "unknown feature category: " ).append( category ) );
            }
        }
                  ...
                  || !strncmp( argv[ i ], "--target=", 9 ) )
        {
            ...
            {
                die( string( "unknown target: " ).append( target_name ) );
            }
        }
        ...
        {
            die( string( "unrecognized option: '" ).append( argv[ i ] ).append( "'" ) );
        }
    }
    ...
    switch ( target )
    {
        ...
        default:
            die( string( "unsupported target: '" ).append( target_to_string( target ) ).append( "'" ) );
            break;
    }
