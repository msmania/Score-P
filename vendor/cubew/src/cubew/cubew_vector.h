...
#ifdef HAVE_ADVANCED_MEMORY /* use own wrappers only if macro is defined */
...
#else /* HAVE_ADVANCED_MEMORY */
#define ADD_NEXT( VECTOR, ELEMENT, ETYPE, MESSAGE ) \
    if ( ( VECTOR )->size == ( VECTOR )->capacity ) { \
        if ( ( VECTOR )->capacity == 0 ) { ( VECTOR )->capacity = 1; } \
        else{ ( VECTOR )->capacity *= 2; } \
        REALLOC( ( VECTOR )->data, ( ETYPE* ), sizeof( ETYPE ) * ( 0 ), sizeof( ETYPE ) * ( ( VECTOR )->capacity ), ( MESSAGE ) ); \
    } \
    ( VECTOR )->data[ ( VECTOR )->size ] = ( ELEMENT ); \
    ( VECTOR )->size++;
#endif
